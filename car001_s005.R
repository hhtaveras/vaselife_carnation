...
topsG<-readRDS(paste0("input/res_", model, "_", map, ".Rds"))

if(class(topsG)[1] == "matrix"){
  topsG<-as.data.frame(topsG)
}
...
for (j in n.tops) {
  ...
  if (j == 0) {
    ...
    st.restrict.marker.data(mar.list = tops$SNP, data.set = "top")
...
#png("figures/corrsAD_50runs.png", width = 12, height = 6, units = "in", res = 360)
#boxplot(corrsAD)
#dev.off()
